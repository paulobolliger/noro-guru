// apps/core/(protected)/custos/artigos/page.tsx
'use client';

import { useState, useEffect } from 'react';
import { Loader2, FileText, DollarSign, File, TrendingUp } from 'lucide-react';
...
export default function CustosArtigosPage() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/costs/artigos?period=${period}`);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3">
            ...
            Análise de Custos - Artigos
          </h1>
          <p className="text-gray-600 mt-2">
            Análise detalhada dos custos de geração de artigos com IA
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow p-6 text-white">
          <div className="flex items-center justify-between mb-2">
            <File size={24} className="opacity-80" />
          </div>
          <div className="text-3xl font-bold">{data.metrics.totalItems}</div>
          <div className="text-sm opacity-90 mt-1">Artigos Gerados</div>
        </div>
