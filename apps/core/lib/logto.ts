...
/**
 * Configuração do SDK Logto para apps/core.
 *
 * baseUrl: URL raiz do apps/core (não a URL do servidor Logto).
 *   - Produção: NEXT_PUBLIC_APP_URL (https://app.noro.guru)
 *   - Dev local: http://localhost:3004 (fallback seguro)
 *
...
export const logtoConfig: LogtoNextConfig = {
  ...
  baseUrl: process.env.LOGTO_BASE_URL ?? process.env.NEXT_PUBLIC_APP_URL ?? 'http://localhost:3004',
