    ...
    return (
        <div className="space-y-6">
            ...
            <div className="surface-card rounded-xl p-6 shadow-sm border border-default">
                <div className="flex items-start justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-sm text-muted">Orçamento # {orcamento.numero_orcamento || orcamento.id.substring(0, 8)}</p>
                            <h1 className="text-3xl font-bold text-primary">{orcamento.titulo}</h1>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button className="px-4 py-2 text-sm font-medium text-primary bg-white/10 rounded-lg hover:bg-white/15 flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="surface-card rounded-xl p-5 shadow-sm border border-default">
                    <p className="text-sm text-muted mb-1">Valor Venda</p>
                    <p className="text-2xl font-bold text-green-600">{formatCurrency(orcamento.valor_total)}</p>
                </div>
                <div className="surface-card rounded-xl p-5 shadow-sm border border-default">
                    <p className="text-sm text-muted mb-1">Margem / Comissão</p>
                ...
                <div className="surface-card rounded-xl p-5 shadow-sm border border-default">
                    ...
                    <p className="text-2xl font-bold text-primary">{orcamento.lead?.nome || 'N/D'}</p>
                </div>
                <div className="surface-card rounded-xl p-5 shadow-sm border border-default">
                    <p className="text-sm text-muted mb-1">Expira em</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="surface-card rounded-xl p-6 shadow-sm border border-default">
                        <h2 className="text-xl font-semibold text-primary mb-4">Itens da Proposta ({itens.length})</h2>
                                ...
                                <tbody className="surface-card divide-y divide-white/5">
                                    {itens.map((item) => (
                                        <tr key={item.id}>
                                            ...
                                            <td className="px-6 py-4 text-sm text-primary">{item.descricao}</td>
                    ...
                    <div className="surface-card rounded-xl p-6 shadow-sm border border-default">
                        <h2 className="text-xl font-semibold text-primary mb-4">Termos e Condições</h2>
                        <p className="whitespace-pre-wrap text-primary text-sm">{orcamento.termos_condicoes || 'Nenhum termo e condição definido para este orçamento.'}</p>
                    </div>
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="surface-card rounded-xl p-6 shadow-sm border border-default">
                        <h2 className="text-xl font-semibold text-primary mb-4">Detalhes da Viagem</h2>
                    ...
                    <div className="surface-card rounded-xl p-6 shadow-sm border border-default">
                        <h2 className="text-xl font-semibold text-primary mb-4">Ações Avançadas</h2>
                        <div className="space-y-3">
                             <button onClick={() => router.push(`/admin/orcamentos/${orcamento.id}/editar`)} className="w-full flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-primary bg-yellow-100 rounded-lg hover:bg-yellow-200">
                                <Edit size={18} /> Editar Conteúdo
                            </button>
                            <button className="w-full flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-primary bg-white/10 rounded-lg hover:bg-white/15">
