...
function LeadItem({ lead }: { lead: Lead }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="bg-white rounded-lg border border-gray-200 p-3 shadow-sm hover:shadow-md transition-all group">
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="font-semibold text-gray-900 mb-2 line-clamp-1">{lead.organization_name || 'Sem nome'}</div>
          ...
          {/* Value, Date and Days */}
          <div className="flex items-center justify-between mb-3 pb-3 border-b border-gray-200 text-xs">
            <div className="flex items-center gap-3">
              {valueFormatted && (
                <div className="font-semibold text-emerald-600">{valueFormatted}</div>
              )}
              {daysInStage !== null && (
          ...
          <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              ...
              className="text-xs px-2 py-1 rounded bg-gray-100 hover:bg-gray-200 transition-colors flex-1 text-gray-700"
            >
              Atribuir
            </button>
            <button
...
export default function KanbanBoardDnd({ stages, groups }: { stages: Column[]; groups: Record<string, Lead[]> }) {
  ...
  function ColumnDroppable({ id, children }: { id: string; children: React.ReactNode }) {
    ...
    return (
      <div
        ref={setNodeRef}
        className={`space-y-2 pr-2 transition-colors ${isOver ? 'bg-blue-50 dark:bg-blue-950/30 ring-2 ring-blue-400 rounded-lg' : ''
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="flex gap-4 overflow-x-auto pb-2">
        {stages.map((col) => (
          <Card key={col.key} className="bg-gray-50 w-80 flex-shrink-0 border border-gray-200">
              ...
              <ScrollArea className="max-h-[70vh]">
                <ColumnDroppable id={col.key}>
                  ...
                  {!(columns[col.key]?.length) && (
                    <div className="text-xs text-gray-500 border-2 border-dashed border-gray-300 rounded-lg p-6 text-center bg-white hover:bg-gray-50 transition-colors">
      ...
      <DragOverlay>
        {activeLead ? (
          <div className="rounded-lg border border-gray-300 p-3 shadow-[0_20px_60px_rgba(0,0,0,0.2)] rotate-3 scale-105 transition-transform bg-white">
            <div className="flex items-start gap-2">
              <GripVertical className="w-4 h-4 text-gray-400 mt-1 flex-shrink-0" />
              <div className="flex-1">
                <div className="font-semibold text-gray-900 mb-2">{activeLead.organization_name || 'Sem nome'}</div>
                <div className="text-xs text-gray-600 mb-2">{activeLead.email || activeLead.phone || '—'}</div>
