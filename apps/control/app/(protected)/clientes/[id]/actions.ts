...
export async function getClienteDetalhes(clienteId: string) {
  ...
  try {
    const [data] = await client`
      ...
      WHERE c.id = ${clienteId} AND c.deleted_at IS NULL
      LIMIT 1
    `;
...
export async function updateCliente(clienteId: string, formData: FormData) {
  ...
  try {
    const updates = {
      ...
      idioma_preferido: formData.get('idioma_preferido') as string || null,
      moeda_preferida: formData.get('moeda_preferida') as string || null,
    ...
    await client`
      ...
      SET 
        ...
        nivel = ${updates.nivel},
        idioma_preferido = ${updates.idioma_preferido},
        moeda_preferida = ${updates.moeda_preferida},
  ...
  try {
    ...
    await client`
      INSERT INTO crm.client_documents (
        cliente_id, tipo, numero, pais_emissor, orgao_emissor, data_emissao, data_validade, status,
        arquivo_url, arquivo_public_id, arquivo_nome, arquivo_tamanho, observacoes
      ) VALUES (
        ${documento.cliente_id}, ${documento.tipo}, ${documento.numero}, ${documento.pais_emissor},
  ...
  try {
    ...
    await client`
      INSERT INTO crm.client_preferences (
        cliente_id, frequencia_viagem, orcamento_medio, estilo_viagem, destinos_favoritos, destinos_desejados,
        ...
        ${preferencias.cliente_id}, ${preferencias.frequencia_viagem}, ${preferencias.orcamento_medio},
    ...
    await client.begin(async (sql) => {
      ...
      await sql`
        INSERT INTO crm.client_addresses (
          cliente_id, tipo, principal, logradouro, numero, complemento, bairro, city, state, cep, pais
        ) VALUES (
          ${endereco.cliente_id}, ${endereco.tipo}, ${endereco.principal}, ${endereco.logradouro},
    ...
    await client.begin(async (sql) => {
      if (updates.principal) {
        await sql`
          UPDATE crm.client_addresses
          SET principal = false
          WHERE cliente_id = ${clienteId} AND id <> ${enderecoId}
        `;
      }
  ...
  try {
    ...
    await client`
      INSERT INTO sales.emergency_contacts (
        cliente_id, nome, parentesco, telefone, email, observacoes
      ) VALUES (
        ${contato.cliente_id}, ${contato.nome}, ${contato.parentesco}, ${contato.telefone}, ${contato.email}, ${contato.observacoes}
      )
    `;
  ...
  try {
    ...
    await client`
      INSERT INTO crm.client_miles (
        cliente_id, companhia, numero_programa, categoria, saldo_estimado, data_validade, observacoes
      ) VALUES (
        ${milhas.cliente_id}, ${milhas.companhia}, ${milhas.numero_programa}, ${milhas.categoria},
