...
export function DespesasClient({ despesas, categorias, contas, tenantId }: DespesasClientProps) {
  ...
  const despesasFiltradas = useMemo(() => {
    return despesas.filter((despesa) => {
      ...
      const matchStatus =
        filtroStatus === 'todos' || despesa.status === filtroStatus;
      const matchMarca =
  ...
  }, [despesas, busca, filtroStatus, filtroMarca, filtroCategoria, dataInicio, dataFim]);
    ...
    return despesasFiltradas.reduce(
      (acc, despesa) => {
        if (despesa.status === 'pago') {
          acc.pago += despesa.valor_brl || 0;
        } else if (despesa.status === 'pendente') {
          acc.pendente += despesa.valor_brl || 0;
        }
        acc.total += despesa.valor_brl || 0;
        return acc;
      },
  ...
  const handleEditar = (despesa: any) => {
    setDespesaSelecionada(despesa);
    setIsFormOpen(true);
  };

  const handleVerDetalhes = (despesa: any) => {
    setDespesaSelecionada(despesa);
  ...
  const handleDeletar = async (despesa: any) => {
    if (!confirm(`Tem certeza que deseja deletar: ${despesa.descricao}?`)) {
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Gerencie suas despesas e saídas financeiras
          </p>
        </div>
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        <div>
          Mostrando <strong>{despesasFiltradas.length}</strong> de{' '}
          <strong>{despesas.length}</strong> despesas
        </div>
      </div>
