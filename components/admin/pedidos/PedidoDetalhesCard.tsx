...
import PedidoCobrancasList from './PedidoCobrancasList'; // NOVO: Importação da lista de cobranças
...
export default function PedidoDetalhesCard({ pedido }: PedidoDetalhesCardProps) {
  ...
  // Exibe o formulário de emissão se o pedido NÃO estiver CONCLUIDO, CANCELADO ou AGUARDANDO_PAGAMENTO (se já houver cobranças)
  const hasPendingCobranca = pedido.cobrancas?.some(c => c.status !== 'PAGO' && c.status !== 'ERRO_API');
  const showCobrancaForm = 
    !hasPendingCobranca &&
    pedido.status !== 'CONCLUIDO' && 
    pedido.status !== 'CANCELADO';
    
  // Verifica se o pedido já está pago/cancelado para desabilitar a edição
  const isFinalizado = pedido.status === 'CONCLUIDO' || pedido.status === 'CANCELADO';
  
  // Condição para exibir o aviso de cobrança já emitida
  const showAvisoCobranca = pedido.status === 'AGUARDANDO_PAGAMENTO' && hasPendingCobranca;

  return (
    <div className="space-y-6">
      {/* ... Detalhes Principais e Cliente (Mantidos) ... */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {cliente ? (
              <>
                <p className="font-semibold">{cliente.nome_completo || 'Cliente sem nome'}</p>
      ...
      {/* ================================================================
        Formulário de Emissão de Cobrança (Exibição Condicional)
        ================================================================
      */}
      {showCobrancaForm && (
          <EmitirCobrancaForm 
      ...
      {showAvisoCobranca && (
          <div className="p-4 bg-yellow-100 text-yellow-800 border-l-4 border-yellow-500 rounded-lg font-medium">
              Aviso: Uma cobrança foi emitida e o pedido está aguardando pagamento. Abaixo, você encontra os detalhes para reenvio.
          </div>
      )}

      {/* ================================================================
        Histórico de Cobranças (NOVO)
        ================================================================
      */}
