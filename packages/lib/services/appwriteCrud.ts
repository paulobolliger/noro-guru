...
export type AppwriteDocument = RuntimeDocument;
...
function unavailable(): never {
  throw new Error(
    'Camada Appwrite desativada. Use packages/db para acesso PostgreSQL/Drizzle nesta fase.',
  );
}

export function createCollectionService<TDocument extends AppwriteDocument>(
...
export function createTenantCollectionService<TDocument extends TenantScopedDocument>(
  ...
  const base = createCollectionService<TDocument>(collectionId);
