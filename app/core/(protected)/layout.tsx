...
export default async function ProtectedAdminLayout({
  ...
  // Se solicitado via subdomínio ou caminho /t/:slug, valida o tenant
  const tenant = await getTenantFromHeaders();
  if (typeof tenant === 'object' && tenant !== null && tenant.status === 'failed') {
    return redirect('/?error=tenant_unavailable');
  }
  ...
  if (!user) {
    return redirect('/core/login?redirect=/core');
  }
